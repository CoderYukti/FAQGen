...
// DOCX upload handling with proper spacing and automatic bullets
  ...
  reader.onload = function (event) {
    ...
    mammoth.convertToHtml({
      ...
      .then(result => {
        ...
        // Remove unwanted <br> in the middle of sentences
        html = html.replace(/([a-z0-9.,])<br>\s*([a-z])/gi, '$1 $2');

        // Add newline after question-ending strong tags
        html = html.replace(/([?:])\s*<\/strong>/g, '$1</strong>\n');

        // Convert <li> items into '- ' format for textarea
        html = html.replace(/<li>(.*?)<\/li>/gi, (_, liContent) => `- ${liContent.trim()}\n`);
        ...
        // Normalize spacing: max 2 line breaks between lines
        const cleaned = plainTextWithTags
          .replace(/\n{3,}/g, '\n\n')
          .trim();

        document.getElementById('faqInput').value = cleaned;
        alert("DOCX file loaded with proper spacing and bullet points preserved.");
      })
      .catch(err => {
...
// Ctrl+B to wrap selected text in <strong>
